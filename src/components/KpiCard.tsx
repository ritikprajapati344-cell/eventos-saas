...
export function KpiCard({ title, value, helper, icon: Icon, tone = "primary" }: KpiCardProps) {
  return (
    <article className="glass-panel min-h-[126px] min-w-0 animate-fade-up rounded-lg p-3.5 transition duration-200 hover:-translate-y-1 hover:border-white/20 sm:p-4">
      <div className="flex min-w-0 items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          <p className="break-words text-sm text-app-muted">{title}</p>
          <p className="mt-2 break-words text-lg font-semibold leading-tight tracking-normal text-white xl:text-xl">{value}</p>
        </div>
